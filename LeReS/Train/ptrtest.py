...
class Rendererer():
    ...
    def __call__(self, img, depth, translation=np.array([0.0, 0., 0.]), rev=False):
        ...
        scene = pytinyrenderer.Renderer()
        instance = scene.create_object_instance(scene.create_mesh(
            vertices,
            _flatten_vectors(g.vertex_normals), [0] * len(g.vertices) * 2,
            g.faces, tex.pixels, tex.width, tex.height, 1.))
        camera = pytinyrenderer.Camera(
            viewWidth=self.w,
            viewHeight=self.h,
            ...
            hfov=45,
            vfov=45)
        light = pytinyrendererlight.Light(
        direction=np.array([0, -1, 0]),
        ambient=1.0,
        diffuse=0.0,
        specular=0.0,
        shadowmap_center=np.zeros(3))
        img = scene.get_camera_image([instance], , camera).rgb
        # mask = d == 0
        # rgb = rgb.copy()
        # rgb[mask] = 0
