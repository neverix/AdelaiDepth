...
def randomSamplingNormal(inputs, targets, masks, sample_num, device="cpu"):
...
def edgeGuidedSampling(inputs, targets, edges_img, thetas_img, masks, h, w, device="cpu"):
    # find edges
...
class EdgeguidedNormalRegressionLoss(nn.Module):
    def __init__(self, point_pairs=10000, cos_theta1=0.3, cos_theta2=0.95, cos_theta3=0.5, cos_theta4=0.86, mask_value=-1e-8, max_threshold=10.1, device="cpu"):
        ...
        self.kernel = torch.tensor(np.array([[1, 1, 1], [1, 1, 1], [
                                   1, 1, 1]], dtype=np.float32), requires_grad=False)[None, None, :, :].to(device)

    def scale_shift_pred_depth(self, pred, gt):
        b, c, h, w = pred.shape
        mask = (gt > self.mask_value) & (
            gt < self.max_threshold)  # [b, c, h, w]
        ...
        pred_scale_shift = pred_scale_shift.permute(
    ...
    def getEdge(self, images):
        ...
        a = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]
                         ).to(self.device).view((1, 1, 3, 3)).repeat(1, 1, 1, 1)
    ...
    def getNormalEdge(self, normals):
        ...
        a = torch.Tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]
                         ).to(self.device).view((1, 1, 3, 3)).repeat(3, 1, 1, 1)
        b = torch.Tensor([[1, 2, 1], [0, 0, 0], [-1, -2, -1]]
                         ).to(self.device).view((1, 1, 3, 3)).repeat(3, 1, 1, 1)
    ...
    def forward(self, pred_depths, gt_depths, images, focal_length):
        ...
        inputs = surface_normal_from_depth(
            pred_depths, focal_length, valid_mask=masks)
        targets = surface_normal_from_depth(
            gt_depths, focal_length, valid_mask=masks)
        # find edges from RGB
        ...
        for i in range(n):
            ...
            loss += torch.sum(torch.abs(target_cos[mask_cos75 & consistency_mask] - input_cos[mask_cos75 & consistency_mask])) / (
                torch.sum(mask_cos75 & consistency_mask)+1e-8)
            loss += torch.sum(torch.abs(target_cos[mask_cos10 & consistency_mask] - input_cos[mask_cos10 & consistency_mask])) / (
                torch.sum(mask_cos10 & consistency_mask)+1e-8)

            # Random Sampling regression
            random_sample_num = torch.sum(
                mask_cos10 & consistency_mask) + torch.sum(torch.sum(mask_cos75 & consistency_mask))
            random_inputs_A, random_inputs_B, random_targets_A, random_targets_B = randomSamplingNormal(
            ...
            loss += torch.sum(torch.abs(random_target_cos - random_input_cos)
                              ) / (random_target_cos.shape[0] + 1e-8)
