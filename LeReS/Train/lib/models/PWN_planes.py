...
class PWNPlanesLoss(nn.Module):
    ...
    def __init__(self, focal_x, focal_y, input_size,
                 ...
                 delta_diff_y=0.007, sample_groups=5000,
                 xyz_mode="uvd", device="cpu"):
        """
        ...
        self.u0 = torch.tensor(
            input_size[1] // 2, dtype=torch.float32).to(device)
        self.v0 = torch.tensor(
            input_size[0] // 2, dtype=torch.float32).to(device)
    ...
    def select_index(self, mask_kp):
        ...
        for i in range(x):
            ...
            if valid_points.shape[0] < select_size * 0.6:
                ...
                repeat_idx = torch.randperm(valid_points.shape[0])[
                    :select_size - valid_points.shape[0]]
    ...
    def form_pw_groups(self, p123, pw):
        ...
        pw_groups = torch.cat(
            [pw1[:, :, :, np.newaxis], pw2[:, :, :, np.newaxis], pw3[:, :, :, np.newaxis]], 3)
        return pw_groups
    ...
    def filter_mask(self, pw_pred):
        ...
        proj_query = xy_diff.view(
            m_batchsize * groups, -1, index).permute(0, 2, 1)  # [bn, 3(p123), 2(xy)]
        proj_key = xy_diff.view(m_batchsize * groups, -1,
                                index)  # [bn, 2(xy), 3(p123)]
        ...
        norm_energy = norm_energy.view(
            m_batchsize * groups, -1)  # [bn, 9(p123)]
        ...
        # ignore near
        mask_x = torch.sum(
            torch.abs(xy_diff[:, :, 0, :]) < self.delta_diff_x, 2) > 0
        mask_y = torch.sum(
            torch.abs(xy_diff[:, :, 1, :]) < self.delta_diff_y, 2) > 0
    ...
    def select_points_groups(self, pred_depth, mask_kp):
        ...
        uvz_groups_pred = self.form_pw_groups(
            p123, uvz_pred)  # [x, N, 3(x,y,z), 3(p1,p2,p3)]
        # mask:[b, n]
        mask_valid_pts = (self.filter_mask(uvz_groups_pred)
                          ).to(torch.bool)  # [x, n]
        mask_valid_batch = p123['valid_batch'].repeat(
            1, mask_valid_pts.shape[1])  # [x, n]
        mask_valid = mask_valid_pts & mask_valid_batch.to(
            self.device)  # [x, n]
        ...
        if self.xyz_mode == 'uvd':
            ...
            pw_groups_pred = self.form_pw_groups(
                p123, xyz_pred)  # [x, N, 3(x,y,z), 3(p1,p2,p3)]
    ...
    def constrain_a_plane_loss(self, pw_groups_pre_i, mask_valid_i):
        ...
        orient_mask = torch.sum(torch.squeeze(
            virtual_normal) * torch.squeeze(pw_groups_pred_i[:, :, 0]), dim=1) > 0
    ...
    def forward(self, gt_depth, pred_depth, mask, focal_length=None):
        ...
        for i in range(B):
            ...
            # [x, 1, h, w] x is the planes number
            planes = [mask_i == m for m in unique_planes if m != 0]
            if len(planes) == 0:
                continue
            # torch.cat(planes, dim=0) #
            mask_planes = torch.stack(planes, dim=0)
            pw_groups_pred, mask_valid = self.select_points_groups(
                pred_depth_i[None, :, :, :], mask_planes)  # [x, N, 3(x,y,z), 3(p1,p2,p3)]
            for j in range(unique_planes.numel()-1):
                mask_valid_j = mask_valid[j, :]
                pw_groups_pred_j = pw_groups_pred[j, :]
                loss_tmp, valid_angles = self.constrain_a_plane_loss(
                    pw_groups_pred_j, mask_valid_j)
...
if __name__ == '__main__':
    ...
    pred_depth = torch.rand([2, 1, 385, 513]).to(self.device)
    gt_depth = torch.rand([2, 1, 385, 513]).to(self.device)
