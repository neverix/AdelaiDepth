...
def load_ckpt(args, model, optimizer=None, scheduler=None, val_err=[]):
    ...
    if os.path.isfile(args.load_ckpt):
        ...
        checkpoint = torch.load(
            args.load_ckpt, map_location=lambda storage, loc: storage, pickle_module=dill)
        model_state_dict_keys = model.state_dict().keys()
        # print(model_state_dict_keys)
        # print(checkpoint.keys())
        try:
            checkpoint_state_dict_noprefix = strip_prefix_if_present(
                checkpoint["model_state_dict"], "module.")
        except KeyError:
            sd = strip_prefix_if_present(
                checkpoint["depth_model"], "module.depth_model.")
            del sd["encoder_modules.encoder.conv1.weight"]
            model.depth_model.load_state_dict(sd, strict=False,
                                              )
        else:
...
def save_images(data, pred, scale=60000.):
    ...
    image_dir = os.path.join(cfg.TRAIN.OUTPUT_ROOT_DIR,
                             "../evaluation", model_name)
    if not os.path.exists(image_dir):
        os.makedirs(image_dir)

    if "kitti" in cfg.DATASET:
        name = data["A_paths"][0].split(
            "/")[-4] + "-" + data["A_paths"][0].split("/")[-1].split(".")[0]
    else:
