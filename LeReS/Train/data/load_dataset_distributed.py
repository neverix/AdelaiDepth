...
def MultipleDataLoaderDistributed(opt, sample_ratio=1):
    ...
    custom_multi_sampler_dist = CustomerMultiDataSampler(
        opt, merged_dataset, opt.world_size, opt.phase)
...
class CustomerMultiDataSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        if diff_size > 0:
            extended_indices_dist = np.append(
                indices_array, indices_array[:diff_size])
        else:
        ...
        rank_indices = extended_indices_dist[offset: offset +
                                             self.num_dist_samples]
    ...
    def create_samplers(self):
        ...
        for dataset_i in self.multi_dataset.datasets:
            ...
            dataset_indices_lists.append(np.random.permutation(
                np.arange(len(dataset_i.curriculum_list))))
        ...
        if "train" == self.phase:
            for data_list in dataset_indices_lists:
                ...
                for i in range(cp-1):
                    tmp = np.concatenate(
                        (tmp, np.random.permutation(data_list)), axis=None)
                tmp = np.concatenate((tmp, np.random.choice(
                    data_list, max_len % size_i, replace=False)), axis=None)
...
def create_multiple_dataset(opt):
    ...
    for name in opt.dataset_list:
        ...
        logger.info("%s : %s is loaded, the data size is %d" %
                    (opt.phase, name, len(dataset)))
        ...
        assert len(
            dataset.curriculum_list) == dataset.data_size, "Curriculum list size not equal the data size!!!"
    max_len = np.max(indices_len)
